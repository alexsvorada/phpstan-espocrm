...
final class DefineTemplateTypeConstantRule implements Rule
{
	...
	public function processNode(Node $node, Scope $scope): array
	{
		...
		foreach ($node->getConstants() as $constStmt) {
			...
			foreach ($constStmt->consts as $const) {
				...
				if ($value instanceof StringScalar) {
					$valid = in_array($value->value, ['Base', 'BasePlus'], true);
				}
			}
		}
		...
		if (!$found) {
			return [
				RuleErrorBuilder::message("Entity must define public const string TEMPLATE_TYPE with value 'Base' or 'BasePlus'.")
		...
		if (!$valid) {
			return [
				RuleErrorBuilder::message("Entity TEMPLATE_TYPE must be 'Base' or 'BasePlus'.")
					->identifier($this->buildIdentifier())
					->build(),
			];
		}
